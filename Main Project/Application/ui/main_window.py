...
class MainWindow(QMainWindow):
    def __init__(self, controller, classes):
        super().__init__()
    ...
    def open_video(self):
        ...
        if path:
            ...
            file_basename = os.path.basename(self.controller.dataset_path)
            self.dataset_label.setText(file_basename)
            # Ao abrir o vídeo, o slider atualiza seu valor máximo com base no total de frames
            self.timeline_slider.setMaximum(self.controller.total_frames - 1)
    ...
    def start_video(self):
        ...
        frame_interval = self.controller.frame_interval
    ...
    def update_playback_frame(self):
        ...
        frame = self.controller.get_next_frame()
        if frame is None:
            self.timer.stop()
            return
    ...
    def slider_released(self):
        ...
        frame = self.controller.seek(frame_number)
    ...
    def update_timeline(self):
        ...
        self.timeline_slider.setValue(self.controller.current_frame)
        self.timeline_slider.blockSignals(False)

        # Atualiza a Label do slider sempre que um novo frame for exibido

        # Tempo atual do vídeo
        current_time = (self.controller.current_frame / self.controller.fps)
        # Tempo total do vídeo
        total_time = (self.controller.total_frames / self.controller.fps)
        # Formatação do tempo atual de vídeo (mm:ss)
        current_video_time = self.controller.format_time(current_time)
        # Formatação do tempo total de vídeo (mm:ss)
        total_video_time = self.controller.format_time(total_time)
    ...
    def open_dataset(self):
        ...
        classes = self.controller.open_dataset(path)
